...
export const createCheckListHistory = AsyncHandler(async (req, res) => {
  ...
  await Promise.all(lastData.map(async(item)=>{
    await CreateNotification({title:"assembly have an error ",description:item.description,senderId:req.currentUser?._id,status:"send",reciverId:admin.id,assembly:item.assembly})
  }))
